...
以上代码的运行结果同上。但是，因为加了一句 `size = property(getSize, setSize)`，使得调用方法是不是更优雅了呢？原来用 `r.getSize()`，现在使用 `r.size`，就好像调用一个属性一样。难道你不觉得眼熟吗？在[《多态和封装》](./211.md)中已经用到过 property 函数了，虽然写法略有差别，但是作用一样。
